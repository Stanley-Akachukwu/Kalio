...
namespace Kalio.Core.Services.Users.Implementation
{
    public class UserRepository : IUserRepository
    {
        ...
        public async Task<CommandResult<AuthenticatedResult>> AuthenticateUserAsync(LoginViewModel model)
        {
            var rsp = new CommandResult<AuthenticatedResult>();
            if (model.Email.Equals("nedum@ymail.com") && model.Password.Equals("nedum@p1234"))
            {
            ...
            //var user = await _userManager.FindByEmailAsync(model.Email); //_users.FirstOrDefault(u=>u.Email==model.Email); 
            ...
            //    userClaims.Add(new Claim(Policies.IsAdminAction, Policies.IsAdminAction));
            //}
        ...
        public async Task<CommandResult<List<string>>> GetPermissionsByUserIdAsync(string userId)
        {
            var rsp = new CommandResult<List<string>>();
            var permissions = new List<string> { "weather.access.read" };
            rsp.Response = permissions;
            return await Task.FromResult(rsp);
        }

        public Task<CommandResult<bool>> RegisterUserAsync(RegisterViewModel model)
        {
        ...
        //     new IdentityUser{Id="822uikolwoiewpp830303j3bb2",Email="nedum@yahoo.com"}
        //};
